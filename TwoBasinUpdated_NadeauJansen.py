...
"""
Update: 
The southern ocean surface buoyancy was simplified to a linear equation 
for easier modifications. Since this simplification affects surface buoyancy  
and isopycnal slopes, the MOC streamfunction appears slightly stronger, 
however this effect is minimal. Updated initial conditions are loaded in 
(about the end of the simulation at equilibrium) for the surface buoyancy 
of the North Atlantic, Atlantic, and Pacific regions.

A 0.002 m/s^2, 0.004 m/s^2, and 0.006 m/s^2 surface buoyancy increases are applied to the 
boundary conditions as a first step for the surface warming simulations, following 
a thermal expansion coefficient of 10^-4 K^-1 (taking high latitudes into consideration).  
As a next step, we will have to diagnose the actual surface buoyancy in the GCM in order to 
reproduce more accurate results in this simplified model.

Additionally, save_iters was created and added 
into the time stepping loop to save the output at a specific time interval. We
then wrote out the arrays into the diagnotic file so we can access and plot
...
the northern sinking region, connected to one of the basins (the "Atlantic").
...
Init=np.load('new_initial_conditions.npz')

# Create a diagnostic file for simulation output
Diag_file=None

# boundary conditions with for ex. constant 0.006 surface buoyancy increase:
bs=0.02#+0.006;
# the minimum surface buoyancy in the GCM in the North Atlantic is 3.5890e-04 (or 0.1795C)
bs_north=0.00036#+0.006; 
bAABW=-0.0011#+0.006; 
...
tau=0.16
# Offset is not included in simplified bs_SO equation
#offset=0.0345*(1-np.cos(np.pi*(5.55e5-1.e5)/8e6));
bs_SO=(((bs-bAABW)/3e6)*y)+bAABW
...
MOC_up_iters=int(np.floor(2.*360*86400/dt)) # # between every updates of the stream function, which is every 2 years. 
# Above is the multiplier for MOC time-step (MOC is updated every MOC_up_iters time steps)
plot_iters= int(np.ceil(500*360*86400/dt))  # plotting frequency (in iterations), done every 500 yrs
save_iters= int(np.ceil(50*360*86400/dt))   # save output frequency every 50 years
total_iters=int(np.ceil(4000*360*86400/dt)) # total number of timesteps, runs for 4000 yrs

z=np.asarray(np.linspace(-4000, 0, 80))

# Create empty arrays to save time-dependent diagnostics
b_Atl_save = np.zeros((len(z), int(total_iters / save_iters)))
b_Pac_save = np.zeros((len(z), int(total_iters / save_iters)))
b_north_save = np.zeros((len(z), int(total_iters / save_iters)))
Psi_SO_Pac_save = np.zeros((len(z), int(total_iters / save_iters)))
Psi_SO_Atl_save = np.zeros((len(z), int(total_iters / save_iters)))
Psi_ZOC_save = np.zeros((len(z), int(total_iters / save_iters)))
Psib_ZOC_save = np.zeros((nb, int(total_iters / save_iters)))
bgrid_ZOC_save = np.zeros((nb, int(total_iters / save_iters)))
Psi_AMOC_save = np.zeros((len(z), int(total_iters / save_iters)))
Psib_AMOC_save = np.zeros((nb, int(total_iters / save_iters)))
bgrid_AMOC_save = np.zeros((nb, int(total_iters / save_iters)))
Psi_SO_save = np.zeros((len(z), int(total_iters / save_iters)))

# updated initial conditions

b_Atl=1.0*Init['b_Atl']#[:,-1]; 
b_Pac=1.0*Init['b_Pac']
b_north=1.0*Init['b_north']
...
SO_Atl=Psi_SO(z=z,y=y,b=b_Atl,bs=bs_SO,tau=tau,L=Latl,KGM=K)
SO_Atl.solve()

# create S.O. overturning model instance for Pacific sector
SO_Pac=Psi_SO(z=z,y=y,b=b_Pac,bs=bs_SO,tau=tau,L=Lpac,KGM=K)
SO_Pac.solve()
...
north= Column(z=z,kappa=kappaeff,b=b_north,bs=bs_north,bbot=bbot,Area=A_north,N2min=N2min)
...
for ii in range(0, total_iters):    
   # update buoyancy profile using isopycnal overturning:
   ...
   if ii%save_iters==0: 
      ...
      b_Atl_save[:, int(ii / save_iters)] = Atl.b
      b_Pac_save[:, int(ii / save_iters)] = Pac.b
      b_north_save[:, int(ii / save_iters)] = north.b
      Psi_SO_Atl_save[:, int(ii / save_iters)] = SO_Atl.Psi
      Psi_SO_Pac_save[:, int(ii / save_iters)] = SO_Pac.Psi
      Psi_ZOC_save[:, int(ii / save_iters)] = ZOC.Psi
      Psib_ZOC_save[:, int(ii / save_iters)] = ZOC.Psib(nb=nb)
      bgrid_ZOC_save[:, int(ii / save_iters)] = ZOC.bgrid
      Psi_AMOC_save[:, int(ii / save_iters)] = AMOC.Psi
      Psib_AMOC_save[:, int(ii / save_iters)] = AMOC.Psib(nb=nb)
      bgrid_AMOC_save[:, int(ii / save_iters)] = AMOC.bgrid
    
   #(various stream function and density profiles):
   if ii%plot_iters==0: 
      # Plot current state:
...
# Plots final solution, basically same but with thicker line width)
...
if Diag_file is not None:
    np.savez(Diag_file, b_Atl_save=b_Atl_save, b_Pac_save=b_Pac_save, 
             b_north_save=b_north_save, Psi_SO_Atl_save=Psi_SO_Atl_save, 
             Psi_SO_Pac_save=Psi_SO_Pac_save, Psi_ZOC_save=Psi_ZOC_save,
             Psib_ZOC_save=Psib_ZOC_save, bgrid_ZOC_save=bgrid_ZOC_save,
             Psi_AMOC_save=Psi_AMOC_save, Psib_AMOC_save=Psib_AMOC_save,
             bgrid_AMOC_save=bgrid_AMOC_save, z=z)
...
#### Create Hovmoller and profile plots below
